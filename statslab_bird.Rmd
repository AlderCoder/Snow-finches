...
for (i in 1:K) {
  test.ind <- which(folds == i)
  rf.i <- randomForest(sex_genetics ~ ., data = df_adult_sub[-test.ind,], ntree = 400, 
                       mtry = 3, importance = TRUE, na.action = na.roughfix)
...
We get an accuracy around **84.5 %**. We tuned the parameter ntree and mtry and with this combiantion we get the best result. In the plot below we see the importance of the different variables. Like in the decision tree, wing and P8 are the most important ones.
...
### Comparison of the coefficient estimates

Here we compare the coefficients of the logistic regression only with the complete cases to the one with the imputed data. The significant coefficients are more or less the same. For the other there is a difference.
```{r estimate comparison, echo = FALSE}
logit_mice <- summary(pooled)
cbind(logit_comp[,1], logit_mice[,2])
```

## Conclusion
In conclusion, the imputation of missing values via MICE and subsequent classification using logistic regression is the best model, yielding around 87 % accuracy for the prediction of the genetic sex of the birds. The two most important variables to measure going forward are the Wing length and the feather length (P8).